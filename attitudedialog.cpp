...
void AttitudeDialog::on_pushButton_clicked()
{
    ...
    QString  SdataStr= "EB 90 0A "+rollAngledataStr+pitchAngledataStr+headingAngledataStr+SdataCheckSumHexStr;
     ...
     QString bookDataStr = tr("装订姿态零位：") + tr("横摇角零位=")+ QString::number(rollAngleFloatStr.toDouble(),'f',4)+tr(" 纵摇角零位=")+ QString::number(pitchAngleFloatStr.toDouble(),'f',4)+tr(" 航向零位=")+ QString::number(headingAngleFloatStr.toDouble(),'f',4)+"\n";
...
void AttitudeDialog::on_pushButton_2_clicked()
{
    ...
    QString  SdataStr= "EB 90 0B "+IMUrollAngledataStr+IMUpitchAngledataStr+IMUheadingAngledataStr+SdataCheckSumHexStr +outputRollAngledataStr + outputPitchAngledataStr + outputHeadingAngledataStr + SdataCheckSumHexStr;
     ...
     QString dateStr =current_date_time.toString("[yyyy-MM-dd hh:mm:ss]");
     QString dateTimeStr = dateStr + "\n";
     QString bookDataStr = tr("装订惯组和输出姿态零位：") + tr("惯组横摇角零位=")+ QString::number(IMUrollAngleFloatStr.toDouble(),'f',4)+tr(" 惯组纵摇角零位=")+ QString::number(IMUpitchAngleFloatStr.toDouble(),'f',4)+tr(" 惯组航向角零位=")+ QString::number(IMUheadingAngleFloatStr.toDouble(),'f',4)+tr(" 输出横摇角零位=")+ QString::number(outputRollAngleFloatStr.toDouble(),'f',4)+tr(" 输出纵摇角零位=")+ QString::number(outputPitchAngleFloatStr.toDouble(),'f',4)+tr(" 输出航向角零位=")+QString::number(outputHeadingAngleFloatStr.toDouble(),'f',4)+"\n";
     QString sendBookDataStr = dateTimeStr+bookDataStr;
