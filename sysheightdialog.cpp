...
void sysHeightDialog::on_pushButton_clicked()
{
    ...
    int sysHeightCheckSumInt = sysHeight_hex.mid(6,2).toInt(nullptr,16) + sysHeight_hex.mid(4,2).toInt(nullptr,16)+sysHeight_hex.mid(2,2).toInt(nullptr,16)+sysHeight_hex.mid(0,2).toInt(nullptr,16) +215;

    QString SdataCheckSumHexStr= QString::number(sysHeightCheckSumInt,16).right(2);

    QString  SdataStr= "EB 90 D7 "+sysHeightDisStr+SdataCheckSumHexStr;
