...
void BookOriDataDialog::on_pushButton_clicked()
{
    ...
    QString  SdataStr= "EB 90 01 "+latdataStr+londataStr+SdataCheckSumHexStr;
     ...
     QString bookDataStr = tr("装订初始数据：") + tr("纬度=")+ QString::number(latFloatStr.toDouble(),'f',4)+tr(" 经度=")+ QString::number(lonFloatStr.toDouble(),'f',4)+"\n";
...
void BookOriDataDialog::on_pushButton_2_clicked()
{
    ...
    QString  SdataStr= "EB 90 03 "+speed0dataStr+SdataCheckSumHexStr;
     ...
     QString dateStr =current_date_time.toString("[yyyy-MM-dd hh:mm:ss]");
     QString dateTimeStr = dateStr + "\n";
     QString bookDataStr = tr("装订初始数据：") + tr("初始速度=")+ QString::number(speed0FloatStr.toDouble(),'f',4)+"\n";
     QString sendBookDataStr = dateTimeStr+bookDataStr;
     emit bookLogCMD(sendBookDataStr);
...
void BookOriDataDialog::on_pushButton_3_clicked()
{
    ...
    QString hang0FloatStr = ui->lineEdit_4->text();
    ...
    QString  SdataStr= "EB 90 02 "+hang0FloatDisStr+SdataCheckSumHexStr;
     ...
     QString dateStr =current_date_time.toString("[yyyy-MM-dd hh:mm:ss]");
     QString dateTimeStr = dateStr + "\n";
     QString bookDataStr = tr("装订初始数据：") + tr("初始航向=")+ QString::number(hang0FloatStr.toDouble(),'f',4)+"\n";
     QString sendBookDataStr = dateTimeStr+bookDataStr;
     emit bookLogCMD(sendBookDataStr);
...
void BookOriDataDialog::on_pushButton_5_clicked()
{
    QByteArray Sdata;

    QString sysHeightStr = ui->lineEdit_5->text();
    //float a = sysHeightStr.toFloat();
    double sysHeightf = sysHeightStr.toDouble() * qPow(2,31) / 100000.0;
    qint32 sysHeightInt32 = qRound64(sysHeightf);
    //qint64 i = *((qint64 *)&f);
    QString sysHeight_str = QString::number(sysHeightInt32,16);
    ...
    int sysHeightCheckSumInt = sysHeight_hex.mid(6,2).toInt(nullptr,16) + sysHeight_hex.mid(4,2).toInt(nullptr,16)+sysHeight_hex.mid(2,2).toInt(nullptr,16)+sysHeight_hex.mid(0,2).toInt(nullptr,16) +17;

    QString SdataCheckSumHexStr= QString::number(sysHeightCheckSumInt,16).right(2);

    QString  SdataStr= "EB 90 11 "+sysHeightDisStr+SdataCheckSumHexStr;
    ...
    QString dateStr =current_date_time.toString("[yyyy-MM-dd hh:mm:ss]");
    QString dateTimeStr = dateStr + "\n";
    QString bookDataStr = tr("装订系统高度数据：") + tr("系统高度=")+ QString::number(sysHeightStr.toFloat(),'f',4)+"\n";
    QString sendBookDataStr = dateTimeStr+bookDataStr;
    emit bookLogCMD(sendBookDataStr);
